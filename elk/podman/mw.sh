#!/usr/bin/env bash
# ELK 日志栈: 多选服务再执行 compose 动作 (零依赖, macOS bash 3.2 兼容; 自 work 组 mw.sh 泛化, 卷前缀取自 name:)
...
# 说明: 勾选会连带依赖 (compose depends_on 语义, 如 fluent-bit 自动带起 clickhouse)
set -uo pipefail
cd "$(dirname "$0")"
...
printf '\033[1mELK 日志栈\033[0m — 空格勾选 · a 全选/清空 · Enter 执行 %s · q 取消\n' "$action"
render
while :; do
